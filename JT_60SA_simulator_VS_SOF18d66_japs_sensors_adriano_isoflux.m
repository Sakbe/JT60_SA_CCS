...
% end
% 
% % For some reason the equilibriu, fopr 8 and 6 gaps has FluxCntrl_001 and 19
% with the new betap linealization
for i = 1:nOfFluxCntrl
...
% minorITER.li = [0 0; 0.1 0; 0.1+3e-3 -0.125*equilDist(2); 0.2 -0.125*equilDist(2); stopTime -0.125*equilDist(2)];
...
t = Input_struct.t;
e = Input_struct.e;
rsens = Input_struct.r_sens(55:99);
zsens = Input_struct.z_sens(55:99);
tsens = Input_struct.theta_sens(55:99)+pi;
pdemesh(p,e,[]), 
hold on
quiver(rsens,zsens,cos(tsens),sin(tsens))
 xlim([1 6])
 xlabel('R [m]')
 ylim([-4 4])
 ylabel('Z [m]')
